const API_BASE = "https://smart-visitor-management-backend.onrender.com/api";

// === Register Employee ===
document.getElementById("registerEmpBtn").addEventListener("click", async () => {
  const name = document.getElementById("empName").value;
  const email = document.getElementById("empEmail").value;
  const department = document.getElementById("empDepartment").value; // ✅ NEW FIELD
  ...
  const res = await fetch(`${API_BASE}/employees`, {
    ...
    body: JSON.stringify({ name, email, department }) // ✅ Include department
  });
...
document.getElementById("getEmpByIdBtn").addEventListener("click", async () => {
  ...
  if (res.status === 404) {
    document.getElementById("singleEmployeeResult").textContent = "Employee not found!";
    return;
  }
...
document.getElementById("registerVisitorBtn").addEventListener("click", async () => {
  ...
  const email = document.getElementById("visitorEmail").value; // ✅ Get email
  const purpose = document.getElementById("visitorPurpose").value;

  if (!name || !phone || !email || !purpose) return alert("All fields required!");

  const res = await fetch(`${API_BASE}/visitors`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ name, phone, email, purpose }) // ✅ Send email
  });
...
document.getElementById("getAllVisitorsBtn").addEventListener("click", async () => {
  ...
  visitors.forEach(visitor => {
    const li = document.createElement("li");
    li.textContent = `ID: ${visitor.visitorId}, Name: ${visitor.name}, Phone: ${visitor.phone}, Purpose: ${visitor.purpose}`;
    list.appendChild(li);
  });
});
...
document.getElementById("checkInBtn").addEventListener("click", async () => {
  ...
  if (!visitorId || !employeeId) return alert("Visitor ID and Employee ID required");

  const res = await fetch(`${API_BASE}/visits/checkin?visitorId=${visitorId}&employeeId=${employeeId}`, {
    method: "POST"
  });
...
document.getElementById("checkOutBtn").addEventListener("click", async () => {
  ...
  const res = await fetch(`${API_BASE}/visits/checkout?visitId=${visitId}`, {
    method: "POST"
  });
